...
export default function TiptapEditor({ content, onChange }: Props) {
  ...
  function addImageChoice() {
    const choice = confirm("OK = Import from your computer\nCancel = Enter a URL");
    if (choice) { fileRef.current?.click(); }
    else { const url = prompt("Image URL:"); if (url) editor?.chain().focus().setImage({ src: url }).run(); }
  }
  ...
  return (
    <div className="tiptap-editor border border-brand-200 rounded-xl overflow-hidden">
      ...
      <div className="flex flex-wrap gap-0.5 p-2 border-b border-brand-100 bg-brand-50/50">
        <B active={editor.isActive("bold")} onClick={() => editor.chain().focus().toggleBold().run()} title="Bold"><b>G</b></B>
        <B active={editor.isActive("italic")} onClick={() => editor.chain().focus().toggleItalic().run()} title="Italic"><i>I</i></B>
        <B active={editor.isActive("underline")} onClick={() => editor.chain().focus().toggleUnderline().run()} title="Underline"><u>S</u></B>
        <B active={editor.isActive("strike")} onClick={() => editor.chain().focus().toggleStrike().run()} title="Strikethrough"><s>B</s></B>
        ...
        <span className="w-px h-6 bg-brand-200 mx-1 self-center" />
        <B active={editor.isActive("bulletList")} onClick={() => editor.chain().focus().toggleBulletList().run()} title="Bullet list">{"\u2022"}</B>
        <B active={editor.isActive("orderedList")} onClick={() => editor.chain().focus().toggleOrderedList().run()} title="Numbered list">1.</B>
        <B active={editor.isActive("blockquote")} onClick={() => editor.chain().focus().toggleBlockquote().run()} title="Quote">{"\u275d"}</B>
        <span className="w-px h-6 bg-brand-200 mx-1 self-center" />
        <B active={editor.isActive({textAlign:"left"})} onClick={() => editor.chain().focus().setTextAlign("left").run()} title="Left">{"\u2190"}</B>
        <B active={editor.isActive({textAlign:"center"})} onClick={() => editor.chain().focus().setTextAlign("center").run()} title="Center">{"\u2194"}</B>
        <B active={editor.isActive({textAlign:"right"})} onClick={() => editor.chain().focus().setTextAlign("right").run()} title="Right">{"\u2192"}</B>
        <span className="w-px h-6 bg-brand-200 mx-1 self-center" />
        <div className="relative">
          <B onClick={() => { setShowColors(!showColors); setShowHighlight(false); }} title="Text color"><span style={{color: editor.getAttributes("textStyle").color || "#000"}}>A</span></B>
          {showColors && <div className="absolute top-full left-0 mt-1 bg-white border border-slate-200 rounded-lg shadow-lg p-2 flex flex-wrap gap-1 w-32 z-50">
            ...
            <button type="button" onClick={() => { editor.chain().focus().unsetColor().run(); setShowColors(false); }} className="w-full text-xs text-slate-400 mt-1 hover:text-slate-600">Default</button>
          </div>}
        </div>
        <div className="relative">
          <B onClick={() => { setShowHighlight(!showHighlight); setShowColors(false); }} title="Highlight">{"\ud83d\udd8d"}</B>
        ...
        <span className="w-px h-6 bg-brand-200 mx-1 self-center" />
        <B onClick={addLink} title="Link">{"\ud83d\udd17"}</B>
        ...
        {editor.isActive("table") && <>
          <B onClick={() => editor.chain().focus().addColumnAfter().run()} title="Column +">C+</B>
          <B onClick={() => editor.chain().focus().deleteColumn().run()} title="Column -">C-</B>
          <B onClick={() => editor.chain().focus().addRowAfter().run()} title="Row +">L+</B>
          <B onClick={() => editor.chain().focus().deleteRow().run()} title="Row -">L-</B>
          <B onClick={() => editor.chain().focus().deleteTable().run()} title="Delete table">{"\ud83d\uddd1"}</B>
          <span className="w-px h-6 bg-brand-200 mx-1 self-center" />
        </>}
        <B onClick={() => editor.chain().focus().undo().run()} title="Undo">{"\u21a9"}</B>
        <B onClick={() => editor.chain().focus().redo().run()} title="Redo">{"\u21aa"}</B>
      </div>
