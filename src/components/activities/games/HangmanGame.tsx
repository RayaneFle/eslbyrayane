...
export default function HangmanGame({ config, onComplete }: { config: any; onComplete: (s: number, details?: any[]) => void }) {
  ...
  function next() {
    ...
    if (isLast) { const details = config.words.map((wd: any, i: number) => ({ question: wd.hint || "Word " + (i+1), userAnswer: i < wi ? "Trouve" : found ? "Trouve" : "Perdu", correctAnswer: wd.word, isCorrect: i < wi ? true : found, imageUrl: wd.imageUrl })); onComplete((w / config.words.length) * 100, details); } else { setWi(i => i + 1); setGuessed(new Set()); }
  }
  ...
  return (
    <div className="bg-white rounded-2xl border border-brand-100 p-6 sm:p-8">
      <div className="flex justify-between items-center mb-6"><p className="text-xs text-brand-400">Word {wi+1}/{config.words.length}</p><p className="text-xs text-red-500 font-semibold">{errors}/{MAX} erreurs</p></div>
