...
function InlineImg({ value, onChange }: { value: string; onChange: (v: string) => void }) {
  ...
  return (
    <div className="flex items-center gap-1">
      ...
      <input value={value||""} onChange={e => onChange(e.target.value)} className="flex-1 border border-slate-100 rounded px-2 py-0.5 text-[10px] outline-none" placeholder="or URL" />
...
export default function NewLessonPage() {
  ...
  return (
    <div className="max-w-3xl">
      <h1 className="font-heading text-2xl font-bold text-slate-900 mb-8">Create une lesson</h1>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  {!createType ? (
                    ...
                    <div className="space-y-3">
                      <div className="flex items-center justify-between">
                        ...
                        <button type="button" onClick={()=>setCreateType("")} className="text-xs text-slate-400">Change</button>
                      </div>
                      ...
                      {createType==="QCM" && <div className="space-y-2">
                        {createConfig?.questions?.map((q:any,qi:number) => (
                          <div key={qi} className="p-3 bg-white rounded-lg space-y-2">
                            ...
                            <input value={q.explanation||""} onChange={e=>{const c={...createConfig};c.questions[qi]={...q,explanation:e.target.value};setCreateConfig({...c});}} className="w-full border border-slate-100 rounded-lg px-3 py-2 text-sm outline-none" placeholder="Explanation" />
                          </div>
                        ))}
                      ...
                      {createType==="TRUE_FALSE" && <div className="space-y-2">
                        ...
                        <button type="button" onClick={()=>{const c={...createConfig};c.questions=[...c.questions,{statement:"",isTrue:true}];setCreateConfig({...c});}} className="w-full py-2 border-2 border-dashed border-slate-200 rounded-lg text-xs text-slate-400 hover:border-brand-400">+ Statement</button>
                      </div>}

                      {createType==="FILL_BLANKS" && <div><p className="text-xs text-slate-400 mb-1">Hide with {"{{mot}}"}</p><textarea value={createConfig?.text||""} onChange={e=>setCreateConfig({...createConfig,text:e.target.value})} rows={3} className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm font-mono outline-none" placeholder={"I {{am}} English."} /></div>}

                      {createType==="MATCHING" && <div className="space-y-2">
                        {createConfig?.pairs?.map((p:any,i:number)=>(<div key={i} className="flex gap-2 items-center"><div className="flex-1 space-y-1"><input value={p.left||""} onChange={e=>{const c={...createConfig};c.pairs[i]={...p,left:e.target.value};setCreateConfig({...c});}} className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm outline-none" placeholder="Left" /><InlineImg value={p.leftImage||""} onChange={v=>{const c={...createConfig};c.pairs[i]={...p,leftImage:v};setCreateConfig({...c});}} /></div><span className="text-slate-300">\u2194</span><div className="flex-1 space-y-1"><input value={p.right||""} onChange={e=>{const c={...createConfig};c.pairs[i]={...p,right:e.target.value};setCreateConfig({...c});}} className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm outline-none" placeholder="Right" /><InlineImg value={p.rightImage||""} onChange={v=>{const c={...createConfig};c.pairs[i]={...p,rightImage:v};setCreateConfig({...c});}} /></div>{createConfig.pairs.length>1&&<button type="button" onClick={()=>{const c={...createConfig};c.pairs=c.pairs.filter((_:any,j:number)=>j!==i);setCreateConfig({...c});}} className="text-red-400 text-xs">x</button>}</div>))}
                        <button type="button" onClick={()=>{const c={...createConfig};c.pairs=[...c.pairs,{left:"",right:""}];setCreateConfig({...c});}} className="w-full py-2 border-2 border-dashed border-slate-200 rounded-lg text-xs text-slate-400 hover:border-brand-400">+ Pair</button>
                      </div>}

                      {createType==="MEMORY" && <div className="space-y-2">
                        {createConfig?.pairs?.map((p:any,i:number)=>(<div key={i} className="flex gap-2 items-center"><div className="flex-1 space-y-1"><input value={p.front||""} onChange={e=>{const c={...createConfig};c.pairs[i]={...p,front:e.target.value};setCreateConfig({...c});}} className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm outline-none" placeholder="Front" /><InlineImg value={p.frontImage||""} onChange={v=>{const c={...createConfig};c.pairs[i]={...p,frontImage:v};setCreateConfig({...c});}} /></div><span className="text-slate-300">\u2194</span><div className="flex-1 space-y-1"><input value={p.back||""} onChange={e=>{const c={...createConfig};c.pairs[i]={...p,back:e.target.value};setCreateConfig({...c});}} className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm outline-none" placeholder="Back" /><InlineImg value={p.backImage||""} onChange={v=>{const c={...createConfig};c.pairs[i]={...p,backImage:v};setCreateConfig({...c});}} /></div>{createConfig.pairs.length>1&&<button type="button" onClick={()=>{const c={...createConfig};c.pairs=c.pairs.filter((_:any,j:number)=>j!==i);setCreateConfig({...c});}} className="text-red-400 text-xs">x</button>}</div>))}
                        <button type="button" onClick={()=>{const c={...createConfig};c.pairs=[...c.pairs,{front:"",back:""}];setCreateConfig({...c});}} className="w-full py-2 border-2 border-dashed border-slate-200 rounded-lg text-xs text-slate-400 hover:border-brand-400">+ Pair</button>
                      </div>}
                      ...
                      {createType==="HANGMAN" && <div className="space-y-2">
                        ...
                        <button type="button" onClick={()=>{const c={...createConfig};c.words=[...c.words,{word:"",hint:""}];setCreateConfig({...c});}} className="w-full py-2 border-2 border-dashed border-slate-200 rounded-lg text-xs text-slate-400 hover:border-brand-400">+ Word</button>
                      </div>}
                      ...
                      {createType==="CATEGORIZE" && <div className="space-y-3">
                        <input value={createConfig?.instruction||""} onChange={e=>setCreateConfig({...createConfig,instruction:e.target.value})} className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm outline-none" placeholder="Instruction" />
                        <p className="text-xs font-bold text-slate-500">Categorys:</p>
        ...
        <div className="flex gap-3">
          <button type="submit" disabled={isSubmitting} className="px-8 py-3 bg-gradient-to-r from-brand-500 to-accent-500 text-white font-semibold rounded-xl hover:shadow-glow disabled:opacity-50 transition-all">{isSubmitting?"Saving...":"Save"}</button>
