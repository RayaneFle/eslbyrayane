            ...
            <div key={block.id}>
              ...
              {showPicker === idx && (
                <div className="bg-white rounded-xl border border-slate-200 shadow-lg p-4 mt-2 max-h-60 overflow-y-auto animate-slide-down">
                  <p className="text-xs font-semibold text-slate-400 mb-2">Choose an activity:</p>
                ...
                <div className="bg-amber-50 rounded-xl border border-amber-200 p-4 mt-2 animate-slide-down">
                  {!createType ? (
                    ...
                    <div className="space-y-3">
                      ...
                      {createType==="QCM" && <div className="space-y-2">
                        {createConfig?.questions?.map((q:any,qi:number) => (
                          <div key={qi} className="p-3 bg-white rounded-lg space-y-2">
                            ...
                            <button type="button" onClick={()=>{const c={...createConfig};c.questions[qi]={...q,options:[...q.options,""]};setCreateConfig({...c});}} className="text-xs text-brand-600 mt-1">+ Answer</button>
                      ...
                      {createType==="FILL_BLANKS" && <div><p className="text-xs text-slate-400 mb-1">Surround with {"{{mot}}"}</p><textarea value={createConfig?.text||""} onChange={e=>setCreateConfig({...createConfig,text:e.target.value})} rows={3} className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm font-mono outline-none" placeholder={"Je {{suis}} English."} /></div>}
                      ...
                      {createType==="SORTING" && <div className="space-y-2">
                        ...
                        <button type="button" onClick={()=>{const c={...createConfig};c.items=[...c.items,""];c.correctOrder=[...c.correctOrder,""];setCreateConfig({...c});}} className="w-full py-2 border-2 border-dashed border-slate-200 rounded-lg text-xs text-slate-400 hover:border-brand-400">+ Item</button>
                      </div>}
