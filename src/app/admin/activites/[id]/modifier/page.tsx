  ...
  return (
    <div className="space-y-1">
      ...
      <div className="flex items-center gap-2">
        ...
        <input value={value || ""} onChange={e => onChange(e.target.value)} className="flex-1 border border-slate-200 rounded-lg px-2 py-1 text-xs outline-none" placeholder="or URL" />
  ...
  return (
    <div className="max-w-3xl">
      <div className="flex items-center gap-3 mb-8">
        ...
        <h1 className="font-heading text-2xl font-bold text-slate-900">Edit: {info.label}</h1>
      </div>
      ...
      <div className="space-y-6">
        <div className="bg-white rounded-2xl border border-brand-100 p-6 space-y-4">
          <div><label className="block text-sm font-medium text-slate-600 mb-1">Title *</label><input type="text" value={title} onChange={e => setTitle(e.target.value)} className="w-full border border-slate-200 rounded-xl px-4 py-2.5 focus:ring-2 focus:ring-brand-400 outline-none" /></div>
        ...
        <div className="bg-white rounded-2xl border border-brand-100 p-6">
          ...
          {type === "TRUE_FALSE" && <div className="space-y-4">{tf.map((q, i) => <div key={i} className="p-4 bg-slate-50 rounded-xl space-y-3">
            ...
            <div className="flex gap-4"><label className="text-sm flex items-center gap-2"><input type="radio" checked={q.isTrue} onChange={() => { const u=[...tf]; u[i]={...u[i],isTrue:true}; setTf(u); }} />True</label><label className="text-sm flex items-center gap-2"><input type="radio" checked={!q.isTrue} onChange={() => { const u=[...tf]; u[i]={...u[i],isTrue:false}; setTf(u); }} />False</label></div>
            {tf.length > 1 && <button type="button" onClick={() => setTf(tf.filter((_:any, j:number) => j !== i))} className="text-xs text-red-500 mt-1">Delete</button>}
          </div>)}<button type="button" onClick={() => setTf([...tf, {statement:"",imageUrl:"",isTrue:true,explanation:""}])} className="w-full py-3 border-2 border-dashed border-slate-200 rounded-xl text-sm text-slate-400 hover:border-brand-400">+ Statement</button></div>}

          {type === "FILL_BLANKS" && <div><p className="text-sm text-slate-400 mb-2">Words a hide : {"{{mot}}"}</p><textarea value={fb} onChange={e => setFb(e.target.value)} rows={6} className="w-full border border-slate-200 rounded-xl px-4 py-3 font-mono text-sm outline-none" /></div>}

          {type === "MATCHING" && <div className="space-y-3">{pairs.map((p, i) => <div key={i} className="p-3 bg-slate-50 rounded-xl space-y-2">
            <div className="grid grid-cols-2 gap-2">
              <input value={p.left||""} onChange={e => { const u=[...pairs]; u[i]={...u[i],left:e.target.value}; setPairs(u); }} className="border border-slate-200 rounded-lg px-3 py-2 text-sm outline-none" placeholder="Left" />
              <input value={p.right||""} onChange={e => { const u=[...pairs]; u[i]={...u[i],right:e.target.value}; setPairs(u); }} className="border border-slate-200 rounded-lg px-3 py-2 text-sm outline-none" placeholder="Right" />
            </div>
            <div className="grid grid-cols-2 gap-2">
              <ImageUpload value={p.leftImage||""} onChange={v => { const u=[...pairs]; u[i]={...u[i],leftImage:v}; setPairs(u); }} label="Left image" />
              <ImageUpload value={p.rightImage||""} onChange={v => { const u=[...pairs]; u[i]={...u[i],rightImage:v}; setPairs(u); }} label="Right image" />
            </div>
            {pairs.length>1 && <button type="button" onClick={() => setPairs(pairs.filter((_,j)=>j!==i))} className="text-xs text-red-500">Delete</button>}
          </div>)}<button type="button" onClick={() => setPairs([...pairs, {left:"",right:"",leftImage:"",rightImage:""}])} className="w-full py-3 border-2 border-dashed border-slate-200 rounded-xl text-sm text-slate-400 hover:border-brand-400">+ Pair</button></div>}

          {type === "MEMORY" && <div className="space-y-3">{memory.map((p, i) => <div key={i} className="p-3 bg-slate-50 rounded-xl space-y-2">
            <div className="grid grid-cols-2 gap-2">
              <input value={p.front||""} onChange={e => { const u=[...memory]; u[i]={...u[i],front:e.target.value}; setMemory(u); }} className="border border-slate-200 rounded-lg px-3 py-2 text-sm outline-none" placeholder="Front" />
              <input value={p.back||""} onChange={e => { const u=[...memory]; u[i]={...u[i],back:e.target.value}; setMemory(u); }} className="border border-slate-200 rounded-lg px-3 py-2 text-sm outline-none" placeholder="Back" />
            </div>
            <div className="grid grid-cols-2 gap-2">
              <ImageUpload value={p.frontImage||""} onChange={v => { const u=[...memory]; u[i]={...u[i],frontImage:v}; setMemory(u); }} label="Front image" />
              <ImageUpload value={p.backImage||""} onChange={v => { const u=[...memory]; u[i]={...u[i],backImage:v}; setMemory(u); }} label="Back image" />
            </div>
            {memory.length>1 && <button type="button" onClick={() => setMemory(memory.filter((_,j)=>j!==i))} className="text-xs text-red-500">Delete</button>}
          </div>)}<button type="button" onClick={() => setMemory([...memory, {front:"",back:"",frontImage:"",backImage:""}])} className="w-full py-3 border-2 border-dashed border-slate-200 rounded-xl text-sm text-slate-400 hover:border-brand-400">+ Pair</button></div>}

          {type === "HANGMAN" && <div className="space-y-2">{words.map((w, i) => <div key={i} className="p-3 bg-slate-50 rounded-xl space-y-2">
            <div className="flex gap-2"><input value={w.word||""} onChange={e => { const u=[...words]; u[i]={...u[i],word:e.target.value}; setWords(u); }} className="flex-1 border border-slate-200 rounded-lg px-3 py-2 text-sm outline-none" placeholder="Word" /><input value={w.hint||""} onChange={e => { const u=[...words]; u[i]={...u[i],hint:e.target.value}; setWords(u); }} className="flex-1 border border-slate-200 rounded-lg px-3 py-2 text-sm outline-none" placeholder="Hint" /></div>
            <ImageUpload value={w.imageUrl||""} onChange={v => { const u=[...words]; u[i]={...u[i],imageUrl:v}; setWords(u); }} label="Image" />
            {words.length > 1 && <button type="button" onClick={() => setWords(words.filter((_:any, j:number) => j !== i))} className="text-xs text-red-500 mt-1">Delete</button>}
          </div>)}<button type="button" onClick={() => setWords([...words, {word:"",hint:"",imageUrl:""}])} className="w-full py-3 border-2 border-dashed border-slate-200 rounded-xl text-sm text-slate-400 hover:border-brand-400">+ Word</button></div>}
          ...
          {type === "CATEGORIZE" && <div className="space-y-4"><input value={catInst} onChange={e => setCatInst(e.target.value)} className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm outline-none" placeholder="Instruction" />
            <div><p className="text-xs font-bold text-slate-500 mb-2">Categorys:</p>{cats.map((c, i) => <div key={i} className="p-3 bg-slate-50 rounded-xl mb-2 space-y-2">
              <input value={c.name||""} onChange={e => { const u=[...cats]; u[i]={...u[i],name:e.target.value}; setCats(u); }} className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm outline-none" placeholder="Category" />
            ...
            </div>)}<button onClick={() => setCats([...cats, {name:"",imageUrl:""}])} className="text-xs text-brand-600 font-medium">+ Category</button></div>
            <div><p className="text-xs font-bold text-slate-500 mb-2">Items:</p>{catItems.map((it, i) => <div key={i} className="p-3 bg-slate-50 rounded-xl mb-2 space-y-2">
              <div className="flex gap-2"><input value={it.text||""} onChange={e => { const u=[...catItems]; u[i]={...u[i],text:e.target.value}; setCatItems(u); }} className="flex-1 border border-slate-200 rounded-lg px-3 py-2 text-sm outline-none" placeholder="Text" /><select value={it.category||""} onChange={e => { const u=[...catItems]; u[i]={...u[i],category:e.target.value}; setCatItems(u); }} className="border border-slate-200 rounded-lg px-3 py-2 text-sm outline-none"><option value="">Cat...</option>{cats.filter(c=>c.name||c.imageUrl).map(c=><option key={c.name||c.imageUrl} value={c.name||c.imageUrl}>{c.name||"(image)"}</option>)}</select></div>
          ...
          {type === "DRAG_DROP" && <div className="space-y-4">
            ...
            <div><p className="text-xs font-bold text-slate-500 mb-2">Items:</p>{ddItems.map((it, i) => <div key={i} className="p-3 bg-slate-50 rounded-xl mb-2 space-y-2">
              <div className="flex gap-2"><input value={it.text||""} onChange={e => { const u=[...ddItems]; u[i]={...u[i],text:e.target.value}; setDdItems(u); }} className="flex-1 border border-slate-200 rounded-lg px-3 py-2 text-sm outline-none" placeholder="Text" /><select value={it.zone||""} onChange={e => { const u=[...ddItems]; u[i]={...u[i],zone:e.target.value}; setDdItems(u); }} className="border border-slate-200 rounded-lg px-3 py-2 text-sm outline-none"><option value="">Zone...</option>{ddZones.filter(z=>z.name||z.imageUrl).map(z=><option key={z.name||z.imageUrl} value={z.name||z.imageUrl}>{z.name||"(image)"}</option>)}</select></div>
              <ImageUpload value={it.imageUrl||""} onChange={v => { const u=[...ddItems]; u[i]={...u[i],imageUrl:v}; setDdItems(u); }} label="Image" />
              {ddItems.length>1 && <button type="button" onClick={() => setDdItems(ddItems.filter((_,j)=>j!==i))} className="text-xs text-red-500">Delete</button>}
        ...
        <div className="flex gap-4">
          <button onClick={save} disabled={saving} className="px-8 py-3 bg-gradient-to-r from-brand-500 to-accent-500 text-white font-semibold rounded-xl hover:shadow-glow disabled:opacity-50 transition-all">{saving ? "Saving..." : "Save"}</button>
          <button onClick={() => router.back()} className="px-8 py-3 bg-slate-50 text-slate-600 font-semibold rounded-xl">Cancel</button>
        </div>
      </div>
    </div>
  );
