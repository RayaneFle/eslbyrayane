...
export default function EditActivityPage() {
  ...
  function buildConfig() {
    switch (type) {
      ...
      case "MATCHING": return { pairs: pairs.filter(p => (p.left || p.leftImage) && (p.right || p.rightImage)), instruction: "Match the elements" };
      ...
      <div className="space-y-6">
        <div className="bg-white rounded-2xl border border-brand-100 p-6 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div><label className="block text-sm font-medium text-slate-600 mb-1">Level</label><select value={level} onChange={e => setLevel(e.target.value)} className="w-full border border-slate-200 rounded-xl px-4 py-2.5 outline-none"><option value="">Tous</option>{LEVELS.map(l => <option key={l} value={l}>{l}</option>)}</select></div>
