    ...
    <div className="max-w-3xl">
      ...
      <form onSubmit={onSubmit} className="space-y-6">
        ...
        <div className="bg-white rounded-2xl border border-brand-100 p-6">
          ...
          {type === "FILL_BLANKS" && <div><p className="text-sm text-slate-400 mb-2">Surround with {"{{mot}}"}</p><textarea value={fb} onChange={e => setFb(e.target.value)} rows={6} className="w-full border border-slate-200 rounded-xl px-4 py-3 font-mono text-sm outline-none" placeholder={"Je {{mange}} une {{pomme}}."} /></div>}
          ...
          {type === "HANGMAN" && <div className="space-y-2">{words.map((w, i) => <div key={i} className="p-3 bg-slate-50 rounded-xl space-y-2">
            ...
            {words.length > 1 && <button type="button" onClick={() => setWords(words.filter((_: any, j: number) => j !== i))} className="text-xs text-red-500 mt-1">Delete word</button>}
          ...
          {type === "WORD_ORDER" && <div className="space-y-3">
            ...
            {sentences.map((s, i) => <div key={i} className="p-3 bg-slate-50 rounded-xl space-y-2">
              ...
              <input value={s.text} onChange={e => { const u=[...sentences]; u[i]={...u[i], text:e.target.value}; setSentences(u); }} className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm outline-none" placeholder="Ex: I am English" />
          ...
          {type === "CATEGORIZE" && <div className="space-y-4"><input value={catInst} onChange={e => setCatInst(e.target.value)} className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm outline-none" placeholder="Instruction" />
            ...
            <div><p className="text-xs font-bold text-slate-500 mb-2">Elements :</p>{catItems.map((it, i) => <div key={i} className="p-3 bg-slate-50 rounded-xl mb-2 space-y-2">
              ...
              <ImageUpload value={it.imageUrl} onChange={v => { const u=[...catItems]; u[i].imageUrl=v; setCatItems(u); }} label="Item image" />
            ...
            </div>)}<button type="button" onClick={() => setCatItems([...catItems, {text:"",category:"",imageUrl:""}])} className="text-xs text-brand-600 font-medium">+ Item</button></div>
          </div>}
        </div>
