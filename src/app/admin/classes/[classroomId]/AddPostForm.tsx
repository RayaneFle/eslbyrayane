...
export default function AddPostForm({ classroomId }: { classroomId: string }) {
  ...
  return (
    <div className="p-4 bg-brand-50 rounded-xl space-y-3">
      ...
      <input value={title} onChange={e => setTitle(e.target.value)} className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm outline-none" placeholder="Title" />
      ...
      {type === "link" && <input value={videoUrl} onChange={e => setVideoUrl(e.target.value)} className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm outline-none" placeholder="Link URL" />}
      <div className="flex gap-2">
