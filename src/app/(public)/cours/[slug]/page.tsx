...
export async function generateMetadata({ params }: { params: { slug: string } }): Promise<Metadata> {
  ...
  return {
    title: course.title + " — Level " + course.level,
    description: shortDesc,
    openGraph: {
      title: course.title + " | ESL Guliston",
      description: shortDesc,
    },
  };
}
...
export default async function CourseDetailPage({ params }: { params: { slug: string } }) {
  ...
  const courseSchema = {
    ...
    "provider": {
      ...
      "name": "ESL Guliston",
      "sameAs": "https://eslguliston.vercel.app",
    },
    "inLanguage": "en",
      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
          ...
          <div className="relative">
            ...
            {nextLesson && canAccess && (
              <Link href={"/cours/" + slug + "/lecon/" + nextLesson.id} className="mt-5 inline-flex items-center gap-3 bg-white text-brand-700 font-semibold rounded-xl px-5 py-3 hover:bg-brand-50 hover:shadow-lg transition-all">
                <div className="text-left">
                  <p className="text-[10px] uppercase tracking-wider text-brand-500 font-bold">{nextLesson.status === "in_progress" ? "Resume" : completedLessons === 0 ? "Start" : "Continue"}</p>
            ...
            {!nextLesson && session?.user && completedLessons === totalLessons && totalLessons > 0 && canAccess && (
              <div className="mt-5 inline-flex items-center gap-3 bg-green-500/30 backdrop-blur text-white font-semibold rounded-xl px-5 py-3 border border-white/30">
                ...
                <div>
                  <p className="text-[10px] uppercase tracking-wider text-white/80 font-bold">Well done</p>
        ...
        {!canAccess ? (
          <div className="bg-white rounded-2xl border border-brand-100 p-8 text-center">
            ...
            <Link href={"/cours/" + slug + "/inscription"} className="inline-block mt-4 px-6 py-2.5 bg-gradient-to-r from-brand-500 to-accent-500 text-white font-semibold rounded-xl hover:shadow-glow transition-all">Enroll</Link>
          </div>
        ) : (
                        ...
                        return (
                          <Link key={l.id} href={"/cours/" + slug + "/lecon/" + l.id}
                            ...
                            {session?.user ? (
                              ...
                              <div className="w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold shrink-0 bg-brand-100 text-brand-600">{li + 1}</div>
                            )}
